...
void sever_UDP_Static(int broadcaston)
{
		...
		/*---- Bind the server C to the  static UDP_PORT number and localhost IP address of the created UDP socket descriptor----*/
...
/*---- Read the file serverC.txt as mentioned in the directory and print the neighbor cost information----*/
...
int main()
{
	...
	/*---- On Screen Messages----*/
	printf("\nThe Server C finishes sending its neighbor information to the Client with TCP port number %d and IP address %s\n",TCP_PORT,peer_ip_address);
