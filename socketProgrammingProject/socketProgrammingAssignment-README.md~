...
2. The code files used in this program are as follows: All the servers have the neighbor cost information when they bootup.
	...
	received information, and this process is continued with all servers sequentially(server B ,server C, server D). And the client computes the network topology and sends 
	to the each server (server A , server B, server C, server D are blocked on recvfrom call) and prints them. Finally, it computes the minimum spanning tree from the 
...
6. Minimum spanning Tree algorithm and Linux Programmer's manual were referred.
